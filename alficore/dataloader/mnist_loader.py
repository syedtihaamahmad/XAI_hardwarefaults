...
class MNIST_dataloader():
    ...
    def datagen_itr(self):
        if self.datagen_iter_cnt < self.dataset_length:
            ...
            self.images, self.labels = next(self.datagen_iter)
...
class CIFAR10_dataloader():
    ...
    # def __init__(self, **kwargs):  
        ...
        **kwargs: Arbitrary keyword arguments.
        ...
        # Fetch test data: total 10000 samples
            self.dataset = datasets.CIFAR10('./data', train=False, download=True, transform=self.transform)
        if self.sampleN is not None:
            self.dataset = torch.utils.data.Subset(self.dataset, np.random.choice(len(self.dataset), self.sampleN, replace=False))
        self.data_loader = torch.utils.data.DataLoader(self.dataset, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers)
        self.classes = classes = ('Airplane', 'Car', 'Bird', 'Cat', 'Deer', 'Dog', 'Frog', 'Horse', 'Ship', 'Truck')
        self.datasetfp = None
        self._image_path = -1
        ...
        self.dataset_length = len(self.dataset)
        self.datagen_iter_cnt = 0
        print("CIFAR10 Dataset loaded in {}; {} dataset - Length : {}".format(self.device, self.dataset_type, len(self.dataset)))
